(function(){
  ...
  let gameSeed;
  if (urlParams.seed) {
    gameSeed = stringToSeed(urlParams.seed);
    console.log(`Using URL seed: ${urlParams.seed} -> ${gameSeed}`);
  } else {
    gameSeed = Date.now() >>> 0;
    console.log(`No URL seed, using random: ${gameSeed}`);
  }
  let mapType = urlParams.type || 'unknown';
  ...
  function generateTerrain() {
    ...
    // 6) Generate inland lakes AFTER all terrain shaping is complete
    const lakeConfigs = [];
    const MIN_LAKE_RADIUS = 4;
    const MAX_LAKE_RADIUS = 7;
    const MIN_LAKE_TILES = 35;
    const MAX_LAKE_TILES = 80;
    const minLakeDist = 22;
    const safeMargin = 25;
    const lakeSafeRadius = 14;

    function isValidLakePosition(x, y, existingLakes) {
      if (x < safeMargin || x >= MAP_W - safeMargin || y < safeMargin || y >= MAP_H - safeMargin) {
        return false;
      }
      ...
      for (const lake of existingLakes) {
        const dx = x - lake.x;
        const dy = y - lake.y;
        if (Math.sqrt(dx * dx + dy * dy) < minLakeDist) return false;
      }

      for (let dy = -lakeSafeRadius; dy <= lakeSafeRadius; dy++) {
        for (let dx = -lakeSafeRadius; dx <= lakeSafeRadius; dx++) {
          const tx = x + dx;
          const ty = y + dy;
          if (tx >= 0 && tx < MAP_W && ty >= 0 && ty < MAP_H) {
            if (height[tx + ty * MAP_W] < WATER_LEVEL) return false;
          }
        }
      }
    ...
    function carveNoiseBasedLake(cx, cy, maxRadius, shapeSeed) {
      const waterTiles = [];
      const searchRadius = maxRadius + 10;
      ...
      for (let dy = -searchRadius; dy <= searchRadius; dy++) {
        for (let dx = -searchRadius; dx <= searchRadius; dx++) {
          ...
          const noise1 = fractalNoise(tx * 0.18 + shapeSeed, ty * 0.18, 3, 2.2, 0.55, seed + 820);
          const noise2 = fractalNoise(tx * 0.35 + shapeSeed + 50, ty * 0.35, 2, 2.0, 0.45, seed + 821);
          const noise3 = fractalNoise(tx * 0.08 + shapeSeed + 200, ty * 0.08, 2, 2.0, 0.6, seed + 822);
          
          const combinedNoise = noise1 * 0.5 + noise2 * 0.3 + noise3 * 0.2;
          
          const dist = Math.sqrt(dx * dx + dy * dy);
          const radiusThreshold = maxRadius * (0.7 + combinedNoise * 0.9);
          
          if (dist < Math.max(3, radiusThreshold)) {
            const falloff = 1 - (dist / Math.max(3, radiusThreshold));
            const depth = 0.35 * Math.pow(falloff, 0.6);
            const idx = tx + ty * MAP_W;
            height[idx] = Math.max(WATER_LEVEL - 0.25, height[idx] - depth);
    ...
    function ensureLakeConnectivity(cx, cy, radius) {
      ...
      for (let dy = -radius - 2; dy <= radius + 2; dy++) {
        for (let dx = -radius - 2; dx <= radius + 2; dx++) {
          ...
          const ty = cy + dy;
          if (tx >= 0 && tx < MAP_W && ty >= 0 && ty < MAP_H) {
            if (height[tx + ty * MAP_W] < WATER_LEVEL) {
              waterSet.add(`${tx},${ty}`);
            }
          }
        }
      ...
      const startKey = waterSet.values().next().value;
      queue.push(startKey);
      ...
      while (queue.length > 0) {
        ...
        for (const [nx, ny] of neighbors) {
          const nkey = `${nx},${ny}`;
          if (waterSet.has(nkey) && !visited.has(nkey)) {
            visited.add(nkey);
            queue.push(nkey);
          }
        }
      ...
      for (const key of waterSet) {
        if (!visited.has(key)) {
          const [x, y] = key.split(',').map(Number);
          height[x + y * MAP_W] = WATER_LEVEL - 0.15;
        }
      }
    }

    function countLakeTilesInArea(cx, cy, radius) {
      let count = 0;
      for (let dy = -radius - 2; dy <= radius + 2; dy++) {
        for (let dx = -radius - 2; dx <= radius + 2; dx++) {
          const tx = cx + dx;
          const ty = cy + dy;
          if (tx >= 0 && tx < MAP_W && ty >= 0 && ty < MAP_H) {
            if (height[tx + ty * MAP_W] < WATER_LEVEL) count++;
          }
        }
      }
      return count;
    ...
    const numLakesTarget = 1 + (rng() < 0.4 ? 1 : 0);
    let validPositions = [];
    for (let y = safeMargin; y < MAP_H - safeMargin; y += 4) {
      for (let x = safeMargin; x < MAP_W - safeMargin; x += 4) {
        if (isValidLakePosition(x, y, [])) {
    ...
    let placedLakes = 0;
    let posIdx = 0;
    while (placedLakes < numLakesTarget && posIdx < validPositions.length) {
      const pos = validPositions[posIdx];
      if (isValidLakePosition(pos.x, pos.y, lakeConfigs)) {
        let radius = MIN_LAKE_RADIUS + Math.floor(rng() * (MAX_LAKE_RADIUS - MIN_LAKE_RADIUS));
        const shapeSeed = Math.floor(rng() * 100000);
        
        carveNoiseBasedLake(pos.x, pos.y, radius, shapeSeed);
        ensureLakeConnectivity(pos.x, pos.y, radius + 3);
        
        let lakeTiles = countLakeTilesInArea(pos.x, pos.y, radius + 3);
        while (lakeTiles < MIN_LAKE_TILES && radius < MAX_LAKE_RADIUS + 4) {
          radius += 1;
          carveNoiseBasedLake(pos.x, pos.y, radius, shapeSeed);
          ensureLakeConnectivity(pos.x, pos.y, radius + 3);
          lakeTiles = countLakeTilesInArea(pos.x, pos.y, radius + 3);
        }
        
        const finalTiles = countLakeTilesInArea(pos.x, pos.y, radius + 4);
        if (finalTiles <= MAX_LAKE_TILES + 20) {
          lakeConfigs.push({ x: pos.x, y: pos.y, radius, shapeSeed, tiles: finalTiles });
          placedLakes++;
        }
      }
      posIdx++;
    ...
    if (lakeConfigs.length === 0 && validPositions.length > 0) {
      const pos = validPositions[Math.floor(validPositions.length / 2)];
      const radius = MIN_LAKE_RADIUS + 2;
      const shapeSeed = Math.floor(rng() * 100000);
      carveNoiseBasedLake(pos.x, pos.y, radius, shapeSeed);
      ensureLakeConnectivity(pos.x, pos.y, radius + 3);
      const tiles = countLakeTilesInArea(pos.x, pos.y, radius + 4);
      lakeConfigs.push({ x: pos.x, y: pos.y, radius, shapeSeed, tiles: tiles });
    }
    ...
    console.log(`Valid lake positions: ${validPositions.length}, Lakes placed: ${lakeConfigs.length}`);
    lakeConfigs.forEach((lake, i) => {
      console.log(`  Lake ${i + 1}: pos(${lake.x}, ${lake.y}), radius=${lake.radius}, tiles=${lake.tiles}`);
    });
  }
  ...
  function init() {
    console.log('WebCity Game Initialized');
    console.log(`Map Type: ${mapType}, Seed: ${gameSeed}`);
    
    const infoDiv = document.getElementById('info');
    if (infoDiv) {
      infoDiv.innerHTML = `Type: ${mapType}<br>Seed: ${urlParams.seed || gameSeed}`;
    }
